...
def compile(){
    stage('Compile') {
            ...
            sh 'yarn install'
            sh 'yarn upgrade @roche/patterns-indicators'
...
def dockerBuild(String env){
    stage('Docker build'){
        ...
        hcp_client = docker.build("dockadmin/hcp-client",".")
    }
}
...
def dockerPush(){
    stage('Docker Push'){
        ...
        docker.withRegistry('http://docker-internal.rochedc.accentureanalytics.com:18123/dockadmin', '69f9f2de-ed6a-406a-8c38-ec546d9e0bd2') {
            ...
            hcp_client.push(currentBuild.displayName)
        }
    }
}
...
def deployTo(String env){
  def NAME = "hcp-client"
  def IMAGE_NAME = "docker.rochedc.accentureanalytics.com/dockadmin/hcp-client"
  def ENV = env
  ...
  stage('Docker deploy to ' + env){
    ...
    try{
        sh "/usr/local/bin/kubectl set image deployment/hcp-client hcp-client=docker.rochedc.accentureanalytics.com/dockadmin/hcp-client:$currentBuild.displayName --namespace=$NAME --kubeconfig=config_$ENV"
    }

    finally{
        sh "/usr/local/bin/kubectl rollout status deployment/hcp-client --namespace=$NAME --kubeconfig=config_$ENV"
        sh "/usr/local/bin/kubectl get deployments --namespace=$NAME --kubeconfig=config_$ENV"
    }
